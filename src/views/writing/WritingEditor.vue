...
async function loadNovelChapters(novelId: number) {
  ...
  try {
    ...
    // Always enter write phase, even if no chapters yet
    novelPhase.value = 'write'
...
onMounted(async () => {
  ...
  if (qType && ['ESSAY', 'DIARY', 'NOVEL'].includes(qType) && !isEditMode.value) {
    // For NOVEL: redirect to existing novel instead of creating a duplicate
    if (qType === 'NOVEL') {
      try {
        const res = await http.get('/novels')
        const novels = (res.data || []) as any[]
        if (novels.length > 0) {
          const latest = novels.sort((a: any, b: any) =>
            new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()
          )[0]
          router.replace({ path: `/writing/${latest.id}`, query: { novelId: latest.id } })
          return
        }
