...
// 概要设计 3.2.4：初期采用单线顺序，按时间升序排保证阅读连贯
...
// 获取最后一个段落 ID，作为单线接龙的 prev_segment_id
...
const loadChain = async () => {
  ...
  try {
    // 对应接口：GET /api/v1/chains/{chainId}
...
const handleContinue = async () => {
  ...
  try {
    // 对应接口：POST /api/v1/chains/{chainId}/segments
    await http.post(`/chains/${chainId}/segments`, {
      body: newSegmentBody.value.trim(),
      prevSegmentId: lastSegmentId.value // 严格遵循单链表结构
    })
    newSegmentBody.value = ''
    await loadChain() // 刷新故事线
  } catch (error: any) {
    ...
    <template v-else-if="chainData">
      ...
      <div class="continue-box">
        ...
        <textarea
          ...
          placeholder="展开你的想象力..."
        ></textarea>
        <button class="btn-submit" @click="handleContinue" :disabled="isSubmitting">
...
.continue-textarea {
  width: 100%; border: 1px solid #dadce0; border-radius: 6px; padding: 16px;
...
.btn-submit {
  float: right; padding: 10px 32px; background: #1a73e8; color: white;
  border: none; border-radius: 6px; font-size: 16px; cursor: pointer; font-weight: 500;
}
